...
(function () {
  ...
  async function populateModels(provider, savedModel, settings) {
    ...
    try {
      ...
      if (
        opencodeModelCache.key === cacheKey &&
        opencodeModelCache.models.length > 0
      ) {
  ...
  function renderMarkdown(text) {
    return MARKED_API
      ? MARKED_API.parse(text, MARKED_OPTIONS)
      : `<p>${text}</p>`;
  }
  ...
  document.addEventListener("mousemove", (e) => {
    ...
    let newW =
      resizeRH === "left" ? resizeAnchorX - pointerX : pointerX - resizeAnchorX;
    let newH =
      resizeRV === "top" ? resizeAnchorY - pointerY : pointerY - resizeAnchorY;
...
Begin.`;
  ...
  async function autoSave() {
    ...
    const opacity =
      parseInt(document.getElementById("ai-opacity-slider").value) / 100;
    const btnOp =
      parseInt(document.getElementById("ai-btn-opacity-slider").value) / 100;
